...
``max_command_ttl`` is required and must be set to a positive duration.
This defines the maximum value for the corresponding ``ttl`` parameter
of command submissions, i.e., the maximum time during which a command can
be deduplicated. 
