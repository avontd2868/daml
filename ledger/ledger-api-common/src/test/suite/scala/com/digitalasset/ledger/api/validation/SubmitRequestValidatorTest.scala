...
class SubmitRequestValidatorTest
  ...
  object api {
    ...
    val ttl = new Duration().withSeconds(10)
    val command =
      Command(
  ...
  object internal {
    ...
    val maxTtl = java.time.Duration.ofDays(1)
    val ttl = Instant.EPOCH.plusSeconds(api.ttl.seconds)
  ...
  "CommandSubmissionRequestValidator" when {
    "validating command submission requests" should {
      ...
      "not allow missing ledgerId" in {
        requestMustFailWith(
          commandsValidator
            .validateCommands(api.commands.withLedgerId(""), internal.submittedAt, internal.maxTtl),
          INVALID_ARGUMENT,
      ...
      "not allow missing submitter" in {
        requestMustFailWith(
          commandsValidator
            .validateCommands(api.commands.withParty(""), internal.submittedAt, internal.maxTtl),
          INVALID_ARGUMENT,
          """Missing field: party"""
        )
      }
